...
export const fetchFilms = createAsyncThunk<Film[], undefined, { extra: Extra }>(
  ...
  async (_arg, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<ShortMovieDto[]>(APIRoute.Films);
...
>(`${NameSpace.Genre}/fetchFilmsByGenre`, async (genre, { extra }) => {
  ...
  const { data } = await api.get<ShortMovieDto[]>(route);
  return adaptFilmsToClient(data);
});
...
export const editFilm = createAsyncThunk<Film, Film, { extra: Extra }>(
  ...
  async (filmData, { extra }) => {
    const { api } = extra;
    const postData = await api.patch<MovieDto>(
...
export const addFilm = createAsyncThunk<Film, CreateFilm, { extra: Extra }>(
  ...
  async (filmData, { extra }) => {
    const { api } = extra;
    const postData = await api.post<MovieDto>(APIRoute.Films, adaptCreateFilmToServer(filmData));
...
export const deleteFilm = createAsyncThunk<Film, string, { extra: Extra }>(
  ...
  async (id, { extra }) => {
    const { api } = extra;
    const { data } = await api.delete<MovieDto>(`${APIRoute.Films}/${id}`);
...
>(`${NameSpace.FavoriteFilms}/fetchFavoriteFilms`, async (_arg, { extra }) => {
  const { api } = extra;
  const { data } = await api.get<ShortMovieDto[]>(APIRoute.Favorite);
...
export const fetchPromo = createAsyncThunk<Film, undefined, { extra: Extra }>(
  ...
  async (_arg, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<MovieDto>(APIRoute.Promo);
...
export const setFavorite = createAsyncThunk<Film, Film['id'], { extra: Extra }>(
  ...
  async (id, { extra }) => {
    const { api } = extra;
    const { data } = await api.patch<MovieDto>(`${APIRoute.Favorite}/${id}`,
      { 'isFavorite': true }
    );
